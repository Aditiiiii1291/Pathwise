"""
Synthetic Data Generator for Pathwise
...
class SyntheticDataGenerator:
    ...
    def _generate_students_roster(self):
        ...
        for i in range(1, self.num_students + 1):
            ...
            # Determined probabilistically by trajectory type with controlled noise.
            # CRITICAL: Fee delay alone (FINANCIAL_CONTEXT_ONLY) has a low baseline dropout rate.
            label_probs = {
    ...
    def _generate_attendance(self):
        ...
        for _, student in self.students_df.iterrows():
            ...
            # Define starting percentage and week-by-week slope
            if traj == "STABLE":
            ...
            for week in range(1, weeks_count + 1):
                ...
                # Expected percentage with noise
                noise = self.rng.normal(0, 2.0)
                expected_pct = np.clip(base_pct + (week - 1) * slope + noise, 0.0, 100.0)

                # Integer attendance conversion to maintain exact mathematical integrity
    ...
    def _generate_marks(self):
        ...
        for _, student in self.students_df.iterrows():
            ...
            # Select 4 subjects for this student
            chosen_subjects = list(self.rng.choice(SUBJECTS_POOL, size=4, replace=False))

            for subject in chosen_subjects:
                # Subject baseline and slope according to trajectory
                if traj == "STABLE":
        ...
        for _, student in self.students_df.iterrows():
            ...
            for sem in semesters_to_record:
                ...
                if traj == "FINANCIAL_CONTEXT_ONLY":
                    # Elevated chance of fee delay for this context group
    ...
    def _generate_attempts(self):
        ...
        for _, student in self.students_df.iterrows():
            ...
            if traj == "STABLE":
                ...
                # Improving students frequently have cleared backlogs
                ...
                # Very few backlogs, academically stable
